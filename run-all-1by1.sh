...
trivy k8s kubernetes-admin@kubernetes --format cyclonedx --output kbom.json || true
...
echo "$SUB_SEPARATOR"
trivy k8s kubernetes-admin@kubernetes --timeout 300m --disable-node-collector --report summary --format json > "$OUTPUT_DIR/trivy-cluster-report/cluster.json" || true
